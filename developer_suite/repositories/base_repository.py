"""Generic CRUD repository base for Developer Suite models.

See this package's ``__init__.py`` for why this is a small,
independent class rather than a reuse of
``repositories.base_repository.BaseRepository`` (same pattern,
different, unrelated model hierarchy).
"""
...
ModelT = TypeVar("ModelT", bound=DeveloperSuiteBaseModel)


class BaseRepository(Generic[ModelT]):
    """Generic CRUD repository for a single Developer Suite ORM model.
    ...
    (see :mod:`developer_suite.services.base_service`).
