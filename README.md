...
- **لوحة تحكم**: إحصائيات لحظية — الموظفون النشطون، الأقسام، حالة الأجهزة، وملخص حضور اليوم (حاضر/متأخر/غائب/إجازة).
- **إدارة الموظفين والأقسام**: بحث، تصنيف هرمي للأقسام (شجرة قابلة لإعادة الترتيب)، توليد رمز QR وباركود تلقائي لكل موظف.
...
- **الأجهزة البيومترية**: دعم بروتوكولات ZKTeco (TCP/UDP) وHikvision (ISAPI)، اختبار اتصال، مزامنة سجلات، ودفع بيانات الموظفين إلى الجهاز.
...
- **الإعدادات**: ملف تعريف الشركة، تفضيلات العرض (المنطقة الزمنية، صيغة التاريخ، العملة)، ونسخ احتياطي/استعادة آمن لقاعدة البيانات (متوافق مع وضع WAL).
...
- **أمان**: كلمات مرور مُجزّأة (bcrypt)، قفل الحساب بعد محاولات فاشلة متكررة، انتهاء الجلسة عند عدم النشاط، وسجل تدقيق شامل.
...
| `BACKUP_AUTO_ENABLED` / `BACKUP_INTERVAL_HOURS` / `BACKUP_RETENTION_COUNT` | `true` / `24` / `14` | سياسة النسخ الاحتياطي التلقائي |
...
لا توجد مجموعة اختبارات آلية (test suite) مضمّنة في المستودع؛ تم التحقق من كل طبقة (النماذج، المستودعات، الخدمات، الأجهزة، وحدات التحكم، وكل شاشة واجهة) يدويًا أثناء التطوير عبر سيناريوهات تشغيل حقيقية ضد قاعدة بيانات SQLite فعلية و`QApplication` حقيقي (بوضع `QT_QPA_PLATFORM=offscreen` للواجهات).

لتشغيل التطبيق في بيئة بلا شاشة (headless) لأغراض الاختبار:
