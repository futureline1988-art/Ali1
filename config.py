...
@dataclass(frozen=True)
class UpdateConfig:
    """Optional software update checking settings.

    This application works fully offline after installation and never
    requires a central server for normal operation — update checking
    is opt-in and off by default (:attr:`enabled` defaults to
    ``False``). When an administrator manually configures an update
    server connection (see :mod:`repositories.update_credential_repository`),
    :attr:`server_url` is the fallback used if that stored credential
    doesn't carry its own URL. There is no automatic background
    schedule; a caller (e.g. a "Check for Updates" action) invokes
    :class:`~updates.checker.UpdateCheckService` manually.
    """
    ...
    def from_env(cls) -> "UpdateConfig":
        ...
        return cls(
            enabled=_env_bool("UPDATES_ENABLED", False),
            server_url=os.getenv("UPDATES_SERVER_URL", cls.server_url),
