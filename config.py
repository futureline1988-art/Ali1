...
class SyncConfig:
    ...
    installation cannot know its company in advance and this screen
    never lists other companies either. Instead, an administrator
    hands each company's employees a unique company code (generated by
    the Developer Suite); the login screen's first-run company-code
    field resolves it with the server (see
    :meth:`~services.subscription_check_service.SubscriptionCheckService.resolve_company_code`)
    and this installation's company is then remembered locally — no
    administrator token or manual device-to-subscription linking is
    involved either way.
    """
